    ...
    <div class="page-card detail-header">
      ...
      <div class="header-main">
        ...
        <div class="header-actions">
          <el-button :type="isFavorite ? 'danger' : 'default'" :icon="isFavorite ? StarFilled : Star" size="large" @click="toggleFav">
            {{ isFavorite ? '已收藏' : '收藏' }}
          </el-button>
...
function toggleFav() {
  ...
  ElMessage.success(jobStore.isFavorite(job.value.id) ? '已收藏该岗位' : '已取消收藏')
}

function apply() {
  if (!job.value) return
  if (jobStore.applyJob(job.value.id)) {
